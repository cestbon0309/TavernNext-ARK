...
async function displayWorldEntries(name, data, navigation = navigation_option.none, flashOnNav = true) {
    ...
    $('#world_popup_export').off('click').on('click', async () => {
        if (name && data) {
            try {
                const response = await fetch('/api/worldinfo/export', {
                    method: 'POST',
                ...
                toastr.success(result.shared ? t`Share panel opened.` : t`World Info exported.`);
            } catch (error) {
                console.error('Error exporting world info:', error);
                toastr.error(t`Failed to export World Info`);
            }
        }
    });
    ...
    if (characters[chid]?.data?.character_book) {
        ...
        if (!accountStorage.getItem(checkKey) && (!worldName || !world_names.includes(worldName))) {
            ...
            if (power_user.world_import_dialog) {
                const html = `<h3>${t`This character has an embedded World/Lorebook.`}</h3>
                <h3>${t`Would you like to import it now?`}</h3>
                ...
                callGenericPopup(html, POPUP_TYPE.CONFIRM, '', { okButton: t`Yes` }).then(checkResult);
            } else {
                toastr.info(
